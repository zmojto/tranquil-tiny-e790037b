        ...
        <div className="relative container max-w-6xl mx-auto px-6">
          ...
          <div className="flex items-baseline gap-2 mb-8">
            <span className="font-serif text-4xl md:text-5xl text-foreground">
              {retreat.price.toLocaleString('sk-SK')} €
            </span>
        ...
        <div className="container max-w-4xl mx-auto px-6">
          ...
          <div className="prose prose-lg max-w-none text-muted-foreground leading-relaxed">
            {retreat.description.split('\n\n').map((paragraph, index) => (
              <p key={index} className={`mb-6 ${paragraph.trim().startsWith('Tento pobyt si môžete objednať') || paragraph.trim().startsWith('Jógový pobyt Pura Vida Energia') || paragraph.trim().startsWith('Jógový pobyt Krpáčovo Soul') ? 'font-bold' : ''}`}>{paragraph}</p>
            ))}
          </div>
        </div>
      </section>
      ...
      {retreat.travelInfo && (() => {
        ...
        <section className="py-16 md:py-24 bg-card">
          <div className="container max-w-4xl mx-auto px-6">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-4 p-5 bg-background rounded-xl border border-primary/10">
