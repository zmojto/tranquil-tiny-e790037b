        ...
        <div className="container max-w-4xl mx-auto px-6">
          ...
          <div className="prose prose-lg max-w-none text-muted-foreground leading-relaxed">
            {retreat.description.split('\n\n').map((paragraph, index) => (
              <p key={index} className={`mb-6 ${paragraph.trim().startsWith('Tento pobyt si môžete objednať') || paragraph.trim().startsWith('Jógový pobyt Pura Vida Energia') ? 'font-bold' : ''}`}>{paragraph}</p>
            ))}
          </div>
        </div>
      </section>
